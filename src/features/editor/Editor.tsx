...
import {
  ...
  Undo, Redo, MoreHorizontal, X, Plus, RotateCw, Users,
  ScanLine, Maximize2, Minimize2,
} from 'lucide-react';
...
function OverflowMenu({
  onTemplates, onQuotation, onTour, onShare, onProjects, onSettings, onCollaborate, onClose,
}: OverflowMenuProps) {
  const items = [
    { icon: Users,          label: 'Colaborar',      action: onCollaborate },
...
interface EditorProps {
  ...
  openAIPromptOnMount?: string;
}

// ─────────────────────────────────────────────────────────────
export function Editor({ onBack, openScanOnMount, openAIPromptOnMount }: EditorProps) {
  ...
  // Auto-open AI Assistant with pre-filled prompt if requested from dashboard
  useEffect(() => {
    if (openAIPromptOnMount) {
      const timer = setTimeout(() => setShowAIAssistant(true), 500);
      return () => clearTimeout(timer);
    }
    return;
  }, [openAIPromptOnMount]);
    ...
    <div className="h-screen flex flex-col bg-slate-950 overflow-hidden">
            ...
            <AnimatePresence>
              {showOverflow && (
                <OverflowMenu
                  ...
                  onCollaborate={() => setShowCollaboration(true)}
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* ── FLOATING ACTION BUTTON — Scanner (desktop) ────── */}
          <motion.button
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ delay: 0.6, type: 'spring', damping: 20, stiffness: 300 }}
            onClick={() => setShowScan(true)}
            className="hidden md:flex absolute bottom-5 right-5 z-20 items-center gap-2.5 px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white rounded-2xl shadow-lg shadow-emerald-500/30 transition-all hover:scale-105 active:scale-95"
            title="Escanear Cômodo com Câmera"
          >
            <div className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center">
              <ScanLine className="w-4.5 h-4.5" />
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold leading-tight">Escanear</p>
              <p className="text-[10px] text-emerald-100 leading-tight">Cômodo com câmera</p>
            </div>
          </motion.button>
      ...
      <AnimatePresence>
        {showAIAssistant && (
          <AIAssistant
            ...
            initialPrompt={openAIPromptOnMount || undefined}
          />
        )}
      </AnimatePresence>
