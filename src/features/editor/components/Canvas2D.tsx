  ...
  return (
    <ToolContext.Provider value={{ previewState, setPreviewState, activeTool: tool }}>
      <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-slate-100">
        ...
        {/* Empty canvas state */}
        {walls.length === 0 && rooms.length === 0 && !previewState && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-5" style={{ zIndex: 5 }}>
            <div className="text-center">
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-5" style={{ background: 'rgba(59,130,246,0.08)', border: '1.5px dashed rgba(59,130,246,0.25)' }}>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(59,130,246,0.5)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="3" y="3" width="18" height="18" rx="2" />
                  <path d="M3 9h18M9 21V9" />
                </svg>
              </div>
              <p className="text-slate-400 font-semibold text-sm mb-1.5">Canvas vazio</p>
              <p className="text-slate-400 text-xs mb-5 max-w-xs" style={{ opacity: 0.6 }}>
                Selecione <strong>Parede</strong> ou <strong>Cômodo</strong> na barra e clique para começar a desenhar
              </p>
              <div className="flex items-center gap-4 justify-center">
                {[['W', 'Paredes'], ['R', 'Cômodo'], ['S', 'Selecionar']].map(([key, label]) => (
                  <div key={key} className="flex items-center gap-1.5">
                    <kbd className="px-2 py-0.5 rounded text-[10px] font-bold" style={{ background: 'rgba(0,0,0,0.08)', border: '1px solid rgba(0,0,0,0.12)', color: '#64748b' }}>{key}</kbd>
