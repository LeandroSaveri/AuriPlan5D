...
import {
  ...
  Briefcase, Sofa, Lamp, Package, Star, TrendingUp, Users,
  User, LogOut, HelpCircle, X, Check, ArrowRight,
  Building2, Trees, Coffee, Grid3X3, ScanLine, Calculator,
  View, BarChart3, Lightbulb, Layers, Award, Palette,
  Grid2X2, List, Filter, MoreVertical, Mic, Send, Menu,
} from 'lucide-react';
...
function Sidebar({
  ...
  return (
    <aside
      className="fixed left-0 top-0 bottom-0 w-[156px] hidden lg:flex flex-col z-40"
...
function TopHeader({
  ...
  return (
    ...
    >
      {/* Mobile: AuriPlan logo */}
      <div className="flex lg:hidden items-center gap-2 flex-1">
        <div className="w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0"
          style={{ background: 'linear-gradient(135deg,#5B8CFF,#8B5CF6)' }}>
          <Home className="w-3.5 h-3.5 text-white" />
        </div>
        <span className="text-white font-bold text-sm tracking-tight">AuriPlan</span>
      </div>
...
const MOBILE_NAV_ITEMS = [
  ...
  { id: 'templates' as Page, label: 'Templates', icon: LayoutTemplate },
  { id: 'library' as Page,   label: 'Biblioteca',icon: BookOpen },
  { id: 'ai' as Page,        label: 'IA',        icon: Sparkles },
];
...
function MobileNav({
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 lg:hidden z-40 flex items-stretch"
      style={{
      ...
      {MOBILE_NAV_ITEMS.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            ...
            style={{ color: active ? '#5B8CFF' : 'rgba(255,255,255,0.4)' }}
          >
            <Icon className="w-5 h-5" />
            <span className="text-[10px] font-medium">{label}</span>
            {id === 'ai' && !active && (
              <span className="absolute mt-[-18px] ml-3 w-1.5 h-1.5 rounded-full bg-purple-500" />
            )}
          </button>
        );
      })}
    ...
    <div className="px-4 py-6 md:px-8 md:py-8 max-w-5xl">
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onNewProject}
          className="flex items-center gap-2.5 px-6 py-3 rounded-xl text-white font-semibold text-sm"
      ...
      <motion.div
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(91,140,255,0.1) 0%, rgba(139,92,246,0.07) 100%)',
          border: '1px solid rgba(91,140,255,0.15)',
        }}
      >
        <p className="text-xs font-semibold mb-3 uppercase tracking-widest" style={{ color: 'rgba(139,92,246,0.85)' }}>
          ✦ Gerar planta com IA
        </p>
        <div className="flex items-center gap-2 rounded-xl p-2 mb-3" style={{
          ...
          <input
            ...
            onKeyDown={e => { if (e.key === 'Enter' && aiInput.trim()) { onAIGenerate(aiInput); setAiInput(''); } }}
            placeholder="Descreva sua planta... ex: casa com 3 quartos, sala americana"
            className="flex-1 bg-transparent text-white placeholder-slate-600 text-sm focus:outline-none px-2 py-1"
          />
          <button
            onClick={() => {}}
            title="Comando de voz (em breve)"
            className="h-9 w-9 flex-shrink-0 flex items-center justify-center rounded-lg transition-all hover:bg-white/10"
            style={{ color: 'rgba(255,255,255,0.4)' }}
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.93 }}
            onClick={() => { if (aiInput.trim()) { onAIGenerate(aiInput); setAiInput(''); } }}
            title="Gerar planta com IA"
            className="h-9 w-9 flex-shrink-0 flex items-center justify-center rounded-lg transition-all"
            style={{
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onScan}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-white text-xs font-semibold flex-shrink-0"
            style={{ background: 'linear-gradient(135deg,#8B5CF6,#EC4899)', boxShadow: '0 4px 16px rgba(139,92,246,0.3)' }}
            title="Escanear cômodo em 3D"
          >
            <ScanLine className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Scanner 3D</span>
          </motion.button>
        </div>
        <div className="flex flex-wrap gap-2">
          {AI_SUGGESTIONS.map(s => (
            <motion.button
              key={s}
              whileTap={{ scale: 0.95 }}
              onClick={() => { setAiInput(s); inputRef.current?.focus(); }}
      ...
      >
        {[
          { icon: Check, iconBg: 'rgba(34,197,94,0.15)', iconColor: '#4ADE80', value: '156', label: 'Designs Concluídos', trend: '+8%', trendUp: true },
          { icon: TrendingUp, iconBg: 'rgba(91,140,255,0.15)', iconColor: '#5B8CFF', value: '48', label: 'Projetos Ativos', trend: '+12%', trendUp: true },
          { icon: Award, iconBg: 'rgba(251,191,36,0.15)', iconColor: '#FBBF24', value: '320h', label: 'Horas Economizadas', trend: '+5%', trendUp: true },
          { icon: Package, iconBg: 'rgba(168,85,247,0.15)', iconColor: '#C084FC', value: '2.4k', label: 'Móveis Adicionados', trend: '+22%', trendUp: true },
        ].map(({ icon: Icon, iconBg, iconColor, value, label, trend, trendUp }, i) => (
          <motion.div
            key={label}
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs font-semibold flex items-center gap-0.5" style={{ color: trendUp ? '#4ADE80' : '#F87171' }}>
                <TrendingUp className="w-3 h-3" />
                {trend}
              </span>
            </div>
            <div className="text-2xl font-bold text-white leading-tight mb-0.5">{value}</div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {TEMPLATE_ITEMS.map((t, i) => (
            <TemplateCard key={t.id} name={t.name} desc={t.desc} image={t.image} onClick={() => onLoadTemplate(t.templateId)} delay={i * 0.05} />
          ))}
        </div>
      </section>
    </div>
...
function LibraryPage() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8 flex flex-col md:flex-row gap-6 max-w-5xl">
      {/* Category sidebar */}
...
function InnerApp() {
  ...
  const handleAIGenerate = (prompt: string) => {
    ...
    setView('loading');
    try { createProject(`IA: ${prompt.slice(0, 30)}`, DEFAULT_OWNER, ''); } catch {}
    goToEditor();
  };
  ...
  if (view === 'editor') {
    return (
      <Editor
        onBack={() => { setView('dashboard'); setOpenScanOnEnter(false); setPendingAIPrompt(''); }}
        openScanOnMount={openScanOnEnter}
        openAIPromptOnMount={pendingAIPrompt}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Spacer that matches fixed sidebar width, keeps main div correctly offset */}
      <div className="hidden lg:block" style={{ width: 156, flexShrink: 0 }} />
      ...
      <MobileNav activePage={page} onPageChange={setPage} />
